...
### Fixed
- **An erratum or supplement can no longer be filed as the paper it corrects.** The rule that binds a DOI-less capture to a pending paper compared titles, authors, years and identifiers, but not whether the document was the work at all — so a correction notice printing `Erratum: <paper title>`, the paper's authors, its year, and its DOI (which is exactly what an erratum prints) satisfied every check and was filed as the original. Automatic binding now abstains on any front matter marking a correction, comment, retraction or supplement, and a footnote merely pointing at a chapter's erratum is still accepted as the chapter. Alongside it: a candidate title that is only a prefix of the printed title (`Foo` against `Foo: A Different Study`) no longer counts as the printed title, a running head or a second-column citation of a title no longer counts as the document's own title, an author surname that appears only in the title or journal line no longer counts as author evidence, and a year is matched as a year rather than as digits that can hide inside a DOI or a grant number. Each of these was a way for the wrong paper to be filed under a right citation, which is the one outcome this project treats as unacceptable.
