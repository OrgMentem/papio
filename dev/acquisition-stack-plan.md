...
## Watchlists, MCP closure, onboarding (2026-07-15, shipped)

- Shipped in papio `b28def6` + `d011f19`.
- **Watchlists**: `papio watch add|list|remove|run` (migration 0005, schema 5) compose the whole pipeline on a cadence — slim OpenAlex search, ownership dedup, capped protocol-clean submission with watch-stable request IDs, auto-import + collection policy, `watch:` batch manifests, notify-on-new-only, serial execution from the scheduler tick, per-watch failure bookkeeping with auto-disable at five.
- **MCP loop closure**: papio_status, papio_actions_list (quarantine path included by design — the calling agent inspects the artifact), papio_actions_resolve (verify_identity only), bounded papio_batch_wait, papio_watch_add/list/remove. Any MCP agent can now run search → batch → wait → review → resolve → watch without shell access.
- **Onboarding**: idempotent `papio init` (interactive + non-interactive) — validated config, bootstrap migrations, bounded zotio detection, native-host install with exact load/grant instructions, daemon + doctor verification, required-step exit codes.
- Live acceptance was three-strikes-and-in, each failure caught by the watch's own bookkeeping or the new taxonomy: oversized abstract-laden discovery responses (→ slim field selection), `identifiers.openalex` leaking into WorkRequests and failing the frozen schema fail-closed (→ allowed kinds only, OpenAlex ID stays in the manifest), and watch submissions missing the auto_import policy (→ set always; app-level regression). Fourth run: watch "algorithm aversion" queued 5 relevant works (trust-repair, algorithmic-management, control-problem literature) into "Trust in AI advice" — including one needs_review artifact human-verified and resolved through the actions surface mid-batch.

## Phase 5 execution record (2026-07-15)

- Shipped in `1260cf0` (resilience suites), `99231ee` (shared internal/batch + papio_acquire_batch), `737d6a6` (release kit + CHANGELOG), `d41ebfc` (reference docs), plus the README prerequisites fix.
- **Resilience suites** (hermetic): clean install from an empty profile through init/bootstrap/doctor to accepted work; schema-1 → 0005 roll-forward with durable rows intact; interrupted-lease recovery without duplicate attempts; draft/future protocol version skew failing closed. One production gap found and fixed minimally: quarantine leftovers were never cleaned — `SweepTerminalQuarantine` now removes them for TERMINAL jobs only (needs_review/awaiting_human inspection files are load-bearing and survive, regression-tested).
- **Release kit**: `scripts/release.sh <version> [--pin-id]` → binaries (papio version-const gap recorded as TODO; zotio ldflag-stamped), extension zip (manifest+dist only; pin injects the public manifest key, private PEM never moves), go-licenses inventories, SBOMs, checksums, release manifest; verified end-to-end with 0.2.0-rc1 including idempotent pinned re-runs. CHANGELOG.md opens at 0.2.0.
- **Reference docs**: docs/{user-guide,agent-guide,config-reference,troubleshooting}.md grounded against live --help and config source; README trimmed to index + quickstart. The MCP gap RefDocs exposed (no batch creation over MCP) was closed the same wave via neutral `internal/batch` shared by CLI and `papio_acquire_batch` — CLI-compatible IDs, so mixed batches interoperate.
- **Gate run (fresh profile, release binaries, documented commands only): PASSED** — `init --non-interactive` on an empty HOME, doctor honest-failing absent Poppler/Tesseract (gate catch: prerequisites were undocumented → README fixed) then 7×PASS with deps, live arXiv OA acquisition parking on year-evidence identity review, quarantine inspection + `actions resolve --accept`, ready, bundle export carrying `acquisition-bundle/1` with `validation.identity: user_confirmed`, clean daemon stop.
- Explicitly out (no credentials / needs approval): signing + notarization, Web Store submission, instsci fork archival.
...
**Result (CDP-observability, corrected):** the three fixes **work**. Achieved SW observability by copying the profile (`Default` + `Local State` → `/tmp/papio-chrome-cdp`, plus the `NativeMessagingHosts/com.orgmentem.papio.json` manifest which is NOT under `--user-data-dir` by default), then launching the Chrome binary with `--user-data-dir=/tmp/papio-chrome-cdp --remote-debugging-port=9333` (custom dir bypasses the Chrome-136 default-profile CDP block; copied cookies decrypt via the app-global Keychain key, so the Example University/JSTOR session carries; reload the extension once so it re-connects the native host from the copied manifest). Driving the papio SW via CDP (`target.worker().evaluate`), a fresh McKnight job **autonomously classified `article`, clicked the JSTOR download button (`download_initiated=true`), and opened the terms modal** — the exact automation that never fired before. So the core post-auth classification bug is fixed and verified.
- The earlier "12 jobs stuck at auth_returned" was dominated by **test-churn artifacts** (repeated daemon/Chrome restarts left tracked `tab_id`s pointing at dead tabs, and session-restored provider tabs came back `"unloaded"` + untracked, so `findByTab` missed them and classify never ran). In a clean profile with the fixes, classification fires.
...
### Cold-start dormancy FIXED (2026-07-16)

`026944b`. An MV3 worker sleeps after ~30s idle; the reconnect backoff uses `setTimeout` (dies with the worker), and the daemon can't wake a dormant worker (the native connection originates from the extension). So a job queued while the worker slept — the watchlist "grows while you sleep" path — never arrived until an unrelated event woke the worker. Fix: register a `chrome.alarms` alarm (1 min) + `onAlarm` listener; the alarm wakes the worker, whose top-level `start()` re-establishes the native connection so queued offers arrive; `onKeepaliveAlarm` reconnects if the port is still down. Added the `alarms` permission + a BridgeDeps seam. Unit-tested (register / reconnect-on-drop / no-op-when-healthy). Bundle loads clean in a fresh profile (no `DidStartWorkerFail`) and `chrome.alarms.create` registers in-extension; a copied profile rejects the new permission via Secure Preferences MAC (copied-profile artifact — real installs auto-grant on reload).
...
### Unattended reliability: auth-stall cap + report/version fixes (2026-07-16, continuation)

Grounded in live evidence: the daemon event log showed job `job_663b3a74…` cycling `job_accept → auth_pending → auth_returned` ~hourly for 2.5h (325 `browser.job_accept` vs 7 `download_complete` over 24h), re-driving ScienceDirect each wake and tripping its "Are you a robot?" bot wall — that is what generated the hourly `observed/sciencedirect-com/*.html` captures (all captchas, none entitled).

- **Auth-drive cap (extension).** Root cause: a warm session that cannot clear the IdP (expired SSO) is re-offered on every daemon hello (SW-restart recovery by design; `Bridge.offered` resets per hello) and re-driven by `reconcileTabs`, with no attempt bound anywhere. `onTabRemoved` cancels a pre-download dead tab, but a job that reaches `auth_returned` (awaiting_download) — a bot-wall/soft landing that never downloads — parks and re-offers forever. Fix: `StoreShape.authAttempts` (persisted per browser session) counts auth drives that never downloaded, debounced per broker-tab id; after `MAX_AUTH_ATTEMPTS` (3), `onJobOffer`/`reconcileTabs` stop opening broker tabs and emit `provider_outcome: human_auth_required` once (non-terminal daemon-side via `OpenHumanAction` — the job stays parked and re-offers on a future warm launch with a fresh budget). A real `download_started` clears the budget so an earlier expired-session streak cannot cap a now-valid job. Deliberately NOT `job_reject` (which transitions the job to `Unavailable` — terminal). Two unit tests reproduce the exact thrash loop and its termination.
- **Batch report: `no_op` = existing item, not in-progress (`internal/batch/report.go`).** `PlanAndApply` returns `no_op` when the exports-ledger idempotency key makes the Zotero apply mutate nothing (the paper is already in the library — e.g. the McKnight `no_op` from the JSTOR proof). The classifier handled `error`/`applied` but dropped `no_op`/`duplicate` to `default → in_progress`, so a deduplicated paper reported as perpetually working. Now classified `existing_item_attached` with its matched parent key. Test added.
- **Single stamped version (`internal/mcpserver/server.go`).** `api.Version` is a `var` stamped by `release.sh` (`-ldflags -X papio/internal/api.Version=…`), but the MCP server reported its own hardcoded `const Version = "0.1.0"`, so a release left the MCP `Implementation.Version` stale. Now sourced from `api.Version` (mcpserver already imports api; no cycle). The `handler.go` "TODO" the earlier plan referenced was already wired — this was the remaining duplicate.

Full Go suite + extension suite (134 pass / typecheck / build) green. ScienceDirect stays deferred — this session's flywheel captures are all Elsevier bot-wall pages, confirming the deferral rationale; the cap now also stops the stalled SD job from generating them.

### Adoption/import durability review (2026-07-16, continuation)

A read-only scout review of the Go daemon's adoption + import path (state confirmed healthy first: queue drained, 46 `ready` jobs, every file-holding adoption dir maps to a `ready`/cancelled job — **no current "downloaded-but-unimported" strand**; the two prior `import=error` jobs recovered to `ready`). It surfaced 10 grounded durability gaps at the loop boundaries. One fixed now; the rest triaged as backlog (deferred deliberately — they need idempotency-aware design, touch daemon-core recovery with real regression risk, or intersect the *foreign/uncommitted* `zotio` ledger).
...
- **#7 P2 — temporary resolver/fetch failures retry with no attempt cap or backoff bound.** A permanently-"temporary" source cycles resolving/fetching forever. Fix direction: persist per-job retry count/age, cap attempts, bounded backoff+jitter, terminal escalation. (Same class of bound as this session's auth-drive cap.)
- **#8 P2 — no stale-lease sweep for parked (`awaiting_human`) adoption leases.** `RecoverStale` only scans resolving/fetching/validating; a crash before adoption transitions leaves an expired lease reclaimable only by the next `AdoptDownload`. Fix direction: include parked leases in an expiry-reclaim sweep, or use a short-lived transactional adoption claim.
...
### Public-release UX execution record (2026-07-18)

Public-release UX push. All changes isolated to `internal/{config,protocol,browser,cli,nativehost,notify,errcat,mcpserver}`, `extension/*`, and docs; sibling-owned `internal/{app,batch,job,daemon,bootstrap}` and `docs/assets/*.svg` untouched (except a forced 3-line `internal/app/app_test.go` type-literal migration). Committed as `5316d8f` (bulk) and `2340160` (MCP category surfacing).
