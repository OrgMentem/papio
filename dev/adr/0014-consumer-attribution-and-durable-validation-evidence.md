...
**Addendum (2026-08-06), on landing this ADR into `main`:** `acquire.submit_v3`
is now ratified. Decision 4's stated reason not to freeze it was that nobody had
asked; the consumer this surface exists for (Inscribi) has since asked, which is
exactly the condition the paragraph above set. Its params are frozen at
`request` / `auto_import` / `force` / `consumer` and pinned in
`TestRatifiedConsumerContract` beside `submit_v2`'s pin; further evolution takes
a new method name per Decision 3. The original paragraph stands as written —
the reasoning was right on the day, and the record of it is why this addendum
