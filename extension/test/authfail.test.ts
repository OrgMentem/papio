...
test("Shibboleth stale request is classified from the title alone", () => {
  const staleURL = "https://idp.example.edu/idp/profile/SAML2/Redirect/SSO?execution=e1s2";
  expect(
    detectAuthFailure(staleURL, "Example University Login Service - Stale Request"),
  ).toBe("stale_sso");
  ...
  expect(detectAuthFailure(staleURL, "Example University Login Service")).toBeUndefined();
