...
export async function agentDOM(request: AgentDOMRequest): Promise<AgentDOMResult> {
  ...
  const validate = () => {
    ...
    if (entry.protocol !== "https:" || current.protocol !== "https:" || entry.username || entry.password || current.username || current.password || current.origin !== entry.origin || current.pathname !== entry.pathname) return false;
    ...
    if (!/^10\.\d{4,9}\/[^\s<>"\u0000-\u001f\u007f]+$/.test(doi) || !citations.length || citations.some(value => value !== doi)) return false;
    ...
    if (Array.from(document.querySelectorAll("input,textarea,select")).some(node => sensitiveField(node) && visible(node))) return false;
    if (Array.from(document.querySelectorAll('iframe,frame,[data-sitekey],[id*="captcha" i],[class*="captcha" i],.cf-turnstile')).some(node => visible(node) && (node.hasAttribute("data-sitekey") || /captcha|turnstile|challenge/i.test(["src", "title", "id", "class", "name"].map(key => node.getAttribute(key)).join(" "))))) return false;
    ...
    for (const dialog of document.querySelectorAll('dialog[open],[role="dialog"],[role="alertdialog"],[aria-modal="true"]')) {
      if (visible(dialog) && human.test(`${dialog.getAttribute("aria-label") ?? ""} ${gateText(dialog)}`)) return false;
    }
    return true;
  };
  if (!validate()) return { status: "blocked" };
  ...
  if (state.binding !== binding || (request.document !== undefined && state.document !== request.document)) return { status: "stale" };
  ...
  const snapshot = () => {
    ...
    return { projection, source, targets };
  };
  ...
  if (request.method === "act") {
    ...
    if (!previous || previous.revision !== request.revision || state.consumed.has(previous.revision) || previous.source !== current.source || !target || previous.targets.get(choice) !== target || !target.isConnected || !current.projection.controls.some(c => c.id === choice && !c.disabled)) return { status: "stale" };
  ...
  if (request.method !== "observe") return { status: "blocked" };
  ...
  if (state.serial !== serial || !fresh || fresh.source !== current.source) return { status: "stale" };
