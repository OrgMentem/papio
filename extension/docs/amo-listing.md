...
When papio loses a tab it opened for a paper, it can offer to reopen that route. By default that offer is an extension page rather than anything drawn in a web page: it reads no web page content, adds nothing to one, and needs no host access. A user who has granted all-sites access can additionally turn on "Show the lost-tab message in the page you are reading", which draws the same message into the page in front of them; it is off by default, revoking all-sites access turns it off again, and when it draws it reads nothing from the page, renders inside a shadow root, and removes itself before reporting the choice. Both routes show one fixed sentence and one button, contain no identifier, title, URL, or job reference, close eight seconds after arriving (or eight after being brought forward), and perform nothing on closing.
