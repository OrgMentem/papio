...
| `optional_host_permissions`: jstor.org, proquest.com, ebsco, springer, sciencedirect, dl.acm.org, wiley, tandfonline, sagepub, psycnet.apa.org | Optional host access (runtime opt-in) | Publisher/provider sites where a licensed PDF may live. Firefox prompts for each domain only when a job actually needs it; none are granted at install. The ScienceDirect grant also asks for `*.sciencedirectassets.com`, the host that serves its PDF viewer, through the `https://*/*` optional pattern; it is removed with ScienceDirect. |
