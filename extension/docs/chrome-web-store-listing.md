...
| `webNavigation` | Observes top-frame navigation events (start, error, tab replacement) only on papio's own handoff tabs, so it can tell a failed or replaced provider navigation from a slow one and bind a PDF download to the tab that started it. It reads URLs of those tabs only, and never page content. |
| `declarativeNetRequestWithHostAccess` | Makes a signed publisher PDF viewer response (for example ScienceDirect's `pdf.sciencedirectassets.com`) download into the job's papio folder instead of opening in the viewer, because the signed link cannot be fetched a second time. Session rules only: each names papio's own handoff tabs and the tabs they open, matches only `application/pdf` responses, sets only `Content-Disposition: attachment`, and is removed once the job has its download. Acts only on hosts the extension already has access to; blocks and redirects nothing. |
