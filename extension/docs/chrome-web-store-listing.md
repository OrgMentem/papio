...
| `webNavigation` | Detects failed or replaced navigation on papio's own handoff tabs, and links a PDF download to its tab. Reads those tabs' URLs only. |
| `declarativeNetRequestWithHostAccess` | Saves a publisher's one-time signed PDF as a download instead of opening it in the viewer. Session rules limited to papio's own tabs and PDF responses; blocks nothing. |
