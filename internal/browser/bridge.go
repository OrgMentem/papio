...
func materializationIdentifierStrategy(w work.Work) string {
	switch {
	case w.DOI != "":
		return "doi"
	case w.PMID != "":
...
func (b *Bridge) reconcileMaterializationProfiles(ctx context.Context, key []byte) error {
	...
	for _, name := range b.cfg.ResolverNames() {
		...
		specs = append(specs, job.InstitutionProfileSpec{
			...
			AuthenticationClaimID: materializationMAC(key, "authentication_claim", name, inst.ShibbolethEntityID),
		})
	}
...
func (b *Bridge) prepareMaterializationCandidate(ctx context.Context, row job.Row) (*job.BrowserCandidate, error) {
	...
	strategy := materializationIdentifierStrategy(row.Work)
	...
	input := job.BrowserCandidateInput{
		...
		EffectContractID:  "browser_tab:institutional_materialization/1",
		Status:            "eligible",
	}
	candidate, err := b.jobs.CreateBrowserCandidate(ctx, input)
	if err != nil {
...
func (b *Bridge) institutionalRoute(ctx context.Context, jobID string, p *protocol.InstitutionalRouteRequestPayload) ([]json.RawMessage, error) {
	...
	if !liveMaterializationClaim(claim, b.now()) ||
		claim.BindingID != p.BindingID || claim.BrowserHolderGeneration != int64(b.epoch) ||
	...
	// Resolve the route only in local memory. The exact profile fence below is
	// the final durable step before this transient URL can be returned; if it
	// fails, this value is discarded and never crosses the browser boundary.
	...
	ordinal, issueErr := b.jobs.IssueMaterializationRoute(ctx, claim.ID, p.BindingID, int64(b.epoch), claim.RouteIssuanceOrdinal)
	if issueErr != nil {
		result.Outcome, result.Detail = "stale", "route issuance fence was lost"
		if !errors.Is(issueErr, job.ErrMaterializationStale) && !errors.Is(issueErr, job.ErrMaterializationConflict) {
			result.Outcome, result.Detail = "error", "route issuance is unavailable"
		}
		return b.frameInstitutionalRoute(jobID, result)
	}
	result.Outcome, result.RouteIssuanceOrdinal, result.URL = "issued", ordinal, routeURL
	result.ClaimID, result.BindingID = claim.ID, p.BindingID
	return b.frameInstitutionalRoute(jobID, result)
}
...
func (b *Bridge) institutionalNavigated(ctx context.Context, jobID string, p *protocol.InstitutionalNavigatedRequestPayload) ([]json.RawMessage, error) {
	...
	result := protocol.InstitutionalNavigatedResponsePayload{RequestID: requestID}
	if p == nil {
		result.Outcome, result.Detail = "stale", "navigation request is missing"
		return b.frameInstitutionalNavigated(jobID, result)
	}
	claim, err := b.jobs.GetMaterializationClaim(ctx, p.ClaimID)
	if err != nil || claim == nil || !liveMaterializationClaim(claim, b.now()) ||
		claim.BindingID != p.BindingID || claim.BrowserHolderGeneration != int64(b.epoch) ||
		claim.MaterializationKind != "browser_tab" {
		result.Outcome, result.Detail = "stale", "claim is fenced to another holder"
		return b.frameInstitutionalNavigated(jobID, result)
	...
	candidate, candidateErr := b.jobs.GetBrowserCandidate(ctx, claim.CandidateID)
	if candidateErr != nil || candidate == nil || candidate.JobID != jobID || p.RouteIssuanceOrdinal < 1 || p.TabID < 0 {
		result.Outcome, result.Detail = "stale", "navigation does not match the current binding"
		return b.frameInstitutionalNavigated(jobID, result)
	}
	_, _, eligibleNow, eligibilityErr := b.currentMaterializationEligibility(ctx, candidate)
	if eligibilityErr != nil {
	...
	if err := b.jobs.AcknowledgeMaterializationNavigation(ctx, claim.ID, p.BindingID, int64(b.epoch), p.RouteIssuanceOrdinal, p.TabID); err != nil {
		result.Outcome, result.Detail = "stale", "navigation fence was lost"
		if !errors.Is(err, job.ErrMaterializationStale) && !errors.Is(err, job.ErrMaterializationConflict) {
			result.Outcome, result.Detail = "error", "navigation acknowledgement is unavailable"
		}
		return b.frameInstitutionalNavigated(jobID, result)
	}
	...
	if b.holder != nil && b.holder.ID != legacySessionID && compareVersion(b.holder.ExtensionVersion, HandoffFocusMinExtensionVersion) >= 0 {
		...
		for _, id := range ids {
			...
			if b.focusPending[id] && b.institutionalMaterializationAvailable() {
				...
				if candidateErr != nil || candidate == nil {
					if candidateErr != nil {
						log.Printf("papio: preparing institutional candidate for %s: %v", id, candidateErr)
					}
					if candidate == nil || errors.Is(candidateErr, job.ErrMaterializationStale) {
						b.clearMaterializationTracking(id)
					}
					continue
				}
				if candidate.Status != "eligible" && candidate.Status != "claimed" && candidate.Status != "materializing" {
					// Succeeded, abandoned, or another terminal candidate
					// state cannot be safely replayed.
					b.clearMaterializationTracking(id)
					continue
				}
				now := b.now()
				...
				if tracked && offerState.CandidateID != candidate.ID {
					// A new attempt/candidate supersedes the old local
					// correlation; only the current candidate may be offered.
					delete(b.materializationOffered, id)
					tracked = false
					newCandidate = true
				}
				if newCandidate {
					// A prior cancellation belongs to the superseded
					// scaffold and must not suppress this candidate's cancel.
				...
				offerFrame, frameErr := b.frame(protocol.MsgInstitutionalCandidateOffer, id,
					protocol.InstitutionalCandidateOfferPayload{
						CandidateID: candidate.ID, MaterializationKind: "browser_tab",
						ExpiresAt: expiresAt.UTC().Format(time.RFC3339),
					})
				if frameErr != nil {
					return nil, frameErr
				}
				b.materializationOffered[id] = materializationOffer{CandidateID: candidate.ID, ExpiresAt: expiresAt}
				out = append(out, offerFrame)
				b.materializationTracked[id] = true
				// Keep focusPending until the claim/status fence observes
				// acknowledgement; a lost frame is retried on the next poll.
				continue
			}
