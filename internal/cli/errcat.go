...
// jobExplanation is an actionable interpretation of why a job is parked or has
// settled without a file: a short, stable category plus one concrete next step
// the user can take. It turns the daemon's internal transition reasons into
// guidance a first-time user can act on without reading source.
type jobExplanation struct {
	Category string
	Guidance string
}

// explainJob maps a job's state and latest transition reason — and, for the
...
func explainJob(state, reason, resolver, accessMode string, cfg config.Config) jobExplanation {
	switch reason {
	case "institutional_handoff":
		return jobExplanation{"login_required",
	...
	case "resolver_temporarily_unavailable", "candidate_temporarily_unavailable":
		return jobExplanation{"retrying",
...
func waitGuidance(state, reason, resolver, accessMode string, cfg config.Config) string {
	...
	exp := explainJob(state, reason, resolver, accessMode, cfg)
	if exp.Category == "" {
		return ""
	}
	return "  [" + exp.Category + "]\n    \u2192 " + exp.Guidance
...
func explainNoAccess(resolver, accessMode string, cfg config.Config) jobExplanation {
	switch accessMode {
	case config.ModeAssisted, config.ModeMaximal:
		if _, ok := cfg.InstitutionFor(resolver); !ok {
			return jobExplanation{"institution_not_configured",
		...
		return jobExplanation{"no_access",
	...
	case config.ModeConservative:
		return jobExplanation{"no_access_conservative",
	...
	return jobExplanation{"no_access", "No legally accessible copy was found for this work."}
}
