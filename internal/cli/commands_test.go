...
func TestActionURLsSelectAwaitingActionsMostRecentAndDryRun(t *testing.T) {
	...
	want := []string{oaURL, browser.OpenURL(base, rows[1].Work)}
	if got := actionURLs(actions, rows, base, 0); !reflect.DeepEqual(got, want) {
		t.Fatalf("URLs = %#v, want %#v", got, want)
	}
	if got := actionURLs(actions, rows, base, 1); !reflect.DeepEqual(got, want[:1]) {
