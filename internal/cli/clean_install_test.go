...
func TestCleanInstallBootstrapsAndAcceptsWork(t *testing.T) {
	...
	if !strings.Contains(output, "SQLite integrity ok; schema version 5") {
		t.Fatalf("init doctor output does not report schema 5:\n%s", output)
	}
	...
	if err != nil || version != 5 {
		t.Fatalf("fresh schema version = %d, %v; want 5", version, err)
	}
