...
func TestCleanInstallBootstrapsAndAcceptsWork(t *testing.T) {
	...
	for _, check := range doctorReport.Checks {
		if check.Name == "database" && check.Status == doctor.Pass && strings.Contains(check.Detail, "schema version 44") {
			databaseReady = true
		}
	}
	if !databaseReady {
		t.Fatalf("fresh-profile doctor database check missing schema version 44: %+v", doctorReport.Checks)
	}
	...
	if err != nil || version != 44 {
		t.Fatalf("fresh schema version = %d, %v; want 44", version, err)
	}
