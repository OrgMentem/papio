...
-- only to caption a parked human action or an inbox row.
--
-- state walks awaiting_file -> quarantined -> job_created, or parks at
-- parked_no_identifier (no front-matter identifier found) or
-- failed_validation (the settled file was not a valid PDF); 'identified' is
-- reserved vocabulary for a future finer-grained pipeline stage, unused by
-- the v1 sweeper. quarantine_path binds the row to the quarantine copy
-- internal/pdf's structural validator inspected. job_id is set once a job
-- exists to carry the grab's outcome (either a freshly created job or an
-- already-live one the extracted identifier deduplicated onto, or a
-- title-only job created solely to host a pdf_identifier_needed human
-- action when no identifier was found).
--
