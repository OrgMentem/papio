...
func TestOpenRefusesSchemaNewerThanBinary(t *testing.T) {
	ctx := context.Background()
	dataDir := t.TempDir()
	setSchemaVersion(t, ctx, dataDir, 44)
	...
	if err == nil {
		t.Fatal("Open accepted schema version 44 with latest embedded migration 43")
	}
	if got, want := err.Error(), "database schema version 44 is newer than this binary supports (43); refusing to open"; !strings.Contains(got, want) {
