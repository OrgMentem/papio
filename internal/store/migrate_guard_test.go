...
func TestOpenRefusesSchemaNewerThanBinary(t *testing.T) {
	ctx := context.Background()
	dataDir := t.TempDir()
	setSchemaVersion(t, ctx, dataDir, 43)
	...
	if err == nil {
		t.Fatal("Open accepted schema version 43 with latest embedded migration 42")
	}
	if got, want := err.Error(), "database schema version 43 is newer than this binary supports (42); refusing to open"; !strings.Contains(got, want) {
