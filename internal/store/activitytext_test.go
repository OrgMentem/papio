...
func TestActivityTextCoversWrittenEventKinds(t *testing.T) {
	for _, tc := range []struct {
		...
		{"drive.paced_open", map[string]any{"action": "openurl_handoff"}, "Opened by the paced drive"},
		{"drive.open_declined", map[string]any{"reason": "not_offerable"}, "Paced open not taken (not offerable)"},
		{"drive.paused", map[string]any{"reason": "sign_in_needed"}, "Paced drive paused — your library sign-in needs you"},
		{"drive.sign_in_stalled", map[string]any{"until": "2026-09-23T20:00:00Z"}, "Sign-in for this paper did not return — the paced drive moved on"},
		{"drive.paused", map[string]any{"reason": "operator"}, "Paced drive paused"},
		...
		// A busy Zotero, as a large sync makes it, needs neither opening nor
		// a restart.
		{"zotio.auto_import", map[string]any{"status": "waiting", "reason": "zotero_busy"}, "Waiting for Zotero desktop (Zotero is busy)"},
