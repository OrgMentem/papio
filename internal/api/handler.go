...
func RouterWithShutdown(system *bootstrap.System, shutdown context.CancelFunc) ipc.Router {
	...
	methods := map[string]ipc.MethodHandler{
		...
		"drive.pause": func(ctx context.Context, raw json.RawMessage) ([]byte, *ipc.RPCError) {
			return drivePause(ctx, raw, system)
		},
		"drive.resume": func(ctx context.Context, raw json.RawMessage) ([]byte, *ipc.RPCError) {
			return driveResume(ctx, raw, system)
		},
