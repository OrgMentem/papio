...
Docs notes: `docs/reference/*` is generated by `cmd/docs-gen` and `docs/changelog/*` are
...
- **Adding a daemon feature flag breaks two hardcoded assertions** in
  `internal/browser/bridge_test.go`: the exact advertised list in the hello-ack test
  (`slices.Equal(payload.Features, …)`) and the `want := append(…)` in
  `TestHelloAckFeatureCapReservesMandatoryFeatures` — whose synthetic-feature slice bound is
  arithmetic on the mandatory-feature count, so it shifts by one every time. Same class of
  trap as the migration `user_version` bumps above.
