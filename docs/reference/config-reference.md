...
| `native_viewer_helper` | executable path | empty | Experimental native saving of an already-loaded Firefox PDF on macOS. Set an absolute path (or `~/...`) to the separately built helper; empty disables it. The helper uses existing macOS Accessibility permission and never requests permission itself. The daemon and extension must both support the save operation. A saved file must pass normal PDF and work-identity validation before acquisition succeeds. Helper distribution and Windows/Linux native drivers are not yet provided. Restart the daemon after changing this setting. |
...
| `auto_import_paused` | boolean | `false` | Pauses all automatic Zotero imports, including retries and jobs that already saved `auto_import = true`. Acquisition and library lookups continue; job policies stay unchanged. Restart the daemon after changing this setting. Set it back to `false` to resume eligible pending imports. Explicit manual plan/apply commands are unaffected. |
