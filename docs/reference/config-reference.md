...
| `exception_tags` | boolean | `false` | Enables the reconciled exception-tag ledger: *papio* maintains `papio:needs-action` and `papio:unavailable` as Zotero *automatic* tags on linked items in the personal library, converging them with current job states on the daemon's maintenance cadence (`papio zotio tags reconcile` runs one pass on demand). Requires `executable` and zotio ≥ 0.13.0. Lifecycle states are never tagged; a tag the user removes by hand is not re-added unless the job state changes. |
