...
| `direct_routes_enabled` | boolean | `true` | Enables *papio*'s compiled-in provider-direct HTTPS route candidates when the connected extension advertises support. Set `false` to disable these routes daemon-wide; institutional OpenURL routing is unchanged. |
...
| `download_adoption_root` | path string | empty | Root for browser-download adoption; adoption is confined to a job subdirectory beneath it. When empty the effective value is `<your download folder>/papio` — on macOS `~/Downloads/papio`, on Linux/BSD the XDG `XDG_DOWNLOAD_DIR` folder (falling back to `~/Downloads`), on Windows the `Downloads` known folder. It must be a `papio` directory inside the folder your browser downloads into: Chrome can only steer a download to a path relative to its own download directory, so any other root adopts nothing at all, silently. `papio init` creates the effective root, and `papio doctor`'s `adoption_root` check fails and names the resolved path when it is unreachable. |
...
| `native_viewer_helper` | removed | — | Removed. It named the macOS helper program that saved a PDF from Firefox's viewer; the extension now saves that PDF itself. A config that still sets it loads, the value is ignored, and `papio doctor` warns with `config_ignored_settings`. Delete the line. |
...
daemon version that recognizes it.
...
| `auto_import_paused` | boolean | `false` | Pauses all automatic Zotero imports, including retries and jobs that already saved `auto_import = true`. Acquisition and library lookups continue; job policies stay unchanged. This also pauses the follow-up retries after an import: a collection filing or abstract fill the Web API refused with 404 is repeated up to four times, and these pause too. Restart the daemon after changing this setting. Set it back to `false` to resume eligible pending imports. Explicit manual plan/apply commands are unaffected. |
| `auto_enrich` | boolean | `true` | Fills a missing DOI and abstract on an auto-imported paper. In `stored` mode, a new paper whose Crossref record has no abstract gets the discovery source's abstract (OpenAlex by default) before Zotero desktop saves it. Any field still missing after the import gets the conservative scoped zotio enrichment of the imported parent. |
...
| `sibling_title_search` | boolean | `false` | OpenAlex fuzzy sibling title search — a 10-credit `search=` query that looks for an open-access copy of a paywalled work under a different DOI. Off by default, because each search costs 10 credits and few searches find a copy that passes validation. When off, the hop makes no request at all and reports that it made none. Set `true` under `[sources.openalex]` to turn it back on. |
