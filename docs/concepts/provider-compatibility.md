...
| Nature.com | `access=Yes` plus the sidebar's rendered `download-pdf` control gates its href; identity comes from `dc.identifier` | `nature` | Verified route | 2026-09-23: entitled news capture | Nature publishes `citation_pdf_url` even on paywalled pages, so it is not used as the entitlement signal. Article-in-Press pages can serve `_reference.pdf` while their citation meta points elsewhere. Every page renders the same PDF control three times; adapter 0.1.0 matched all three and refused every plan as ambiguous until 0.2.0 scoped it to the sidebar. News items (`10.1038/d41586-…`) have no `citation_doi`, so the DOI is read from `dc.identifier`. The public article endpoint returned `%PDF` bytes; the entitled news PDF is not yet downloaded live. |
...
| Science (AAAS) | Full-access `data-article-access` state plus the rendered `#downloadPdfUrl` anchor to `/doi/pdf/<doi>`, read by href | `science` | Unverified | 2026-09-23: entitled page capture | The institution's resolver sent both measured Science attempts to the journal archive (`/loi/science`), not the article; that page stays unknown. `https://doi.org/<doi>` reaches the article, so `10.1126` DOIs are offered publisher-first. Science's own sign-in entry (`/action/ssostart?idp=<entityID>`) returned to the entitled article without a prompt while the institution session was live. The anchor's href must carry the requested DOI. No denied or signed-out article page is captured yet, so those states stay unknown, and the file bytes are not yet confirmed live. |
| ScienceDirect | Chrome: rendered View PDF control opens the provider viewer. On Chrome 128+, the signed viewer response in a handoff tab downloads into the job binding instead of rendering, with no second request; an older Chrome downloads the viewer link once. If that fails or the viewer still renders, choose **Send this PDF**, then the viewer’s **Download** button. Firefox: you click View PDF yourself (see below); in the handoff tab, or a tab it opens, the extension keeps a copy of the signed viewer response while the viewer shows it and saves that copy into the job binding, with no second request. If the copy is incomplete or the PDF opened before the tab was armed, the viewer’s **Download** button keeps your own copy only | `sciencedirect` | Unverified | 2026-09-20: viewer and manual guidance verified | A fresh isolated direct-provider probe reached the correct eight-page viewer without an Open action, then retained a manual-download task and displayed the matching popup guidance. No file was downloaded or adopted. Earlier signed-URL re-fetches and a fresh complete article href requested through Chrome downloads returned non-PDF content. The manual task releases browser capacity and grants no download authority until Send PDF binds the current document. The adapter supports the captured access-bar and content-actions layouts, binds the page DOI, and scopes controls away from recommended articles. Adapter 0.8.2 also recognizes the captured paired access-bar sign-in layout and refuses disabled PDF controls, including after planning. A purchase control beside institutional access is sign-in evidence, not proof of no entitlement. |
...
clicks it and then uses **Send PDF to papio** to adopt the file. The exception
is a signed PDF viewer such as ScienceDirect's: when the human's click opens it
in *papio*'s handoff tab, the extension saves a copy of that one response itself
(see [PDF viewers that need a manual download](browser-handoff.md#pdf-viewers-that-need-a-manual-download)).
Direct `href`,
