...
| ScienceDirect | Chrome: rendered View PDF control opens the provider viewer. Signed viewer links require **Send this PDF**, then the viewer’s **Download** button. Firefox remains human-assisted | `sciencedirect` | Unverified | 2026-09-20: PDF displayed; automatic adoption failed | An entitled subscription probe reached the correct PDF after institutional access, but re-fetching its signed URL returned HTML. No validated artifact reached `ready`. Automatic viewer adoption now observes the existing signed-link restriction and provides manual-delivery guidance; this is not proof of acquisition. The adapter supports the captured access-bar and content-actions layouts, binds the page DOI, and scopes controls away from recommended articles. A purchase control beside institutional access is sign-in evidence, not proof of no entitlement. |
