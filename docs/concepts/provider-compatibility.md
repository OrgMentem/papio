...
| ScienceDirect | Chrome: rendered access-bar **View PDF** anchor opens the provider viewer; viewer adoption captures its PDF. Firefox: human-assisted, because the route requires a click download | `sciencedirect` | Unverified | Not live-verified | The access-bar control has **two** measured hrefs, and the adapter accepts both: `/pdfft` on an entitled subscription article, and `<pii>/pdf` on an entitled open-access one (measured live 2026-08-26, doi 10.1016/j.sbspro.2014.01.1251, `aria-disabled="false"`). Requiring `/pdfft` alone classified every open-access article `unknown`. Both selectors stay scoped to `.accessbar .ViewPDF >`: on that same page the only other `/pdfft` hrefs belong to three recommended sibling articles, so the scoping — not the path — is what prevents filing a different paper. The adapter keeps the work window visible, makes the paper the active tab inside it, and reloads because ScienceDirect does not hydrate that control while minimized or behind another paper. No retained validated artifact has reached `ready`, so the exact route is not yet **Verified working**: reaching one needs a route that lands on the article page itself, and the resolver answers this DOI with a direct `/pdf` URL that redirects past it to the signed CDN file. The no-entitlement state was re-verified live on 2026-08-26 after a fresh institutional sign-in: ScienceDirect replaced `.PurchasePDF` with a labelled `/getaccess/pii/<pii>/purchase` anchor, which adapter 0.6.0 recognizes. |
