...
| ChemRxiv | Rendered article-toolbar PDF link, bound to the primary self-citation DOI and exact preprint version | `chemrxiv` | Fixture-tested | 2026-09-20 | The page omits `citation_doi`; its version-of-record DOI is a different identifier. Metadata alone, hidden download controls, supplements and other versions remain insufficient. Live download validation is pending. |
...
| ScienceDirect | Chrome: rendered View PDF control opens the provider viewer. Signed viewer links require **Send this PDF**, then the viewer’s **Download** button. Firefox remains human-assisted | `sciencedirect` | Unverified | 2026-09-20: viewer and manual guidance verified | A fresh isolated direct-provider probe reached the correct eight-page viewer without an Open action, then retained a manual-download task and displayed the matching popup guidance. No file was downloaded or adopted. Earlier signed-URL re-fetches returned HTML. The manual task releases browser capacity and grants no download authority until Send PDF binds the current document. The adapter supports the captured access-bar and content-actions layouts, binds the page DOI, and scopes controls away from recommended articles. A purchase control beside institutional access is sign-in evidence, not proof of no entitlement. |
