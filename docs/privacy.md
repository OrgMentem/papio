...
- **No browser credentials leave the page.** You enter institutional credentials and complete MFA or CAPTCHA in your browser. The extension never reads a password field, a cookie, or your browsing history, and it never stores or transmits a username or a session token. To tell whether your library session is live, the check it runs inside the configured library page does read that page's own visible affordances and its `localStorage` and `sessionStorage` entries, including whether a session token found there has expired and whether it carries an identity claim. Every value is discarded in the page: only the resulting `signed in` / `signed out` / `unknown` classification comes back, and nothing else is stored or sent.
...
- **Institution-session checks follow your local keep-warm mode.** The default **While papio has work** mode checks the configured library resolver only while institutional papers are active. **Always for current library** keeps that resolver warm even when no paper currently needs it. **Off** stops automatic resolver checks. When a paper is waiting for sign-in, a completed HTTPS navigation on that paper's declared publisher host can schedule a check of the configured library resolver. This publisher-triggered path reads no publisher page content. The publisher page cannot set the popup's signed-in or signed-out verdict; that comes only from the configured library page. If *papio*'s own resolver tab is still paused on an identity-provider page, the trigger leaves that page open and creates a muted background resolver tab for the check. The replacement surfaces only when the resolver itself says that sign-in is still required. A tracked *papio* tab returning from authentication can still provide the existing, same-origin release evidence for queued work, subject to current resolver access. Other hosts, queued-only work, missing institution bindings, and ambiguous institutions do not authorize this trigger. No new landing URL, page-derived publisher data, title, path, query, fragment, cookie, credential, or session token enters extension storage or a native message.
...
- **The loss toast prefers the page when all-sites access is active.** When *papio* loses a tab it opened for a paper, it can offer to reopen that route. With all-sites access granted, "Show the lost-tab message in the page you are reading" is on by default and draws the message into the page in front of you. Turn it off to use the small *papio* window instead. Revoking all-sites access turns the in-page route off, and re-granting access does not restore an explicit opt-out. Both routes show the same fixed sentence, one button, and *papio*'s own mark. They carry no identifier, title, URL, provider name, or job reference, disappear eight seconds after arriving, and commit nothing when they go. Neither appears while a *papio* surface holds focus.
- **The in-page loss message reads nothing.** The all-sites grant lets *papio* reach the current HTTPS page. The loss-toast setting selects the integrated route or the separate window. When *papio* draws the message, it adds one element and reads nothing from the page. No selector runs, no page text is collected, and nothing about the page is stored or sent. The message renders inside a shadow root, so the page's scripts and styles do not reach it. It removes itself before reporting your choice. It is never drawn into a page *papio* is driving, a non-HTTPS page, or a PDF. A withdrawn grant, blocked injection, or unsafe page always falls back to *papio*'s own window.
...
**Browser storage for page scanning.** Page scanning requires an explicit click for each scan. The browser stores only the restart-safe page-bulk cohort record described above, with no path, query, fragment, page title, or bearer value.
