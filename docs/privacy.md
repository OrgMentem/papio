...
- **The loss toast is a small papio window, not a page overlay.** When *papio* loses a tab it opened for a paper, it can open a small *papio* window offering to reopen that route. The window is one of *papio*'s own pages: it reads nothing from any web page, injects nothing into any page, and needs no site access. It shows one fixed sentence and one button, carries no identifier, title, URL, provider name, or job reference, closes itself eight seconds after it arrives, or eight seconds after you bring it forward, and commits nothing when it closes. It never appears while a *papio* window already has your focus.
