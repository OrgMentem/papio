...
- **The loss toast is a small papio window, unless you ask for it in the page.** When *papio* loses a tab it opened for a paper, it can offer to reopen that route. By default the offer arrives in a small *papio* window. That window is one of *papio*'s own pages: it reads no web page, adds nothing to one, and needs no site access. If you have granted all-sites access **and** turned on "Show the lost-tab message in the page you are reading", *papio* draws that same message into the page in front of you instead. Both routes show the same one fixed sentence, one button, and *papio*'s own mark, carry no identifier, title, URL, provider name, or job reference, disappear eight seconds after arriving, and commit nothing when they go. Neither appears while a *papio* window already has your focus.
- **The in-page loss message reads nothing, and is off until you turn it on.** The in-page route is off by default and needs two separate choices: all-sites access, which you grant for reaching provider pages, and this preference, which is what allows *papio* to draw there. Revoking all-sites access turns the preference off again. When it does draw, *papio* adds one element to the page and reads nothing from it: no selector runs, no page text is collected, and nothing about the page is stored or sent. The message renders inside a shadow root, so the page's own scripts and styles do not reach it and it stays out of any serialized copy of the page. It removes itself before reporting what you chose, it is never drawn into a page *papio* is itself driving, into a page that is not HTTPS, or into a PDF, and a diagnostic page capture taken while it is on screen contains no trace of it.
