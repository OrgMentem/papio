...
| `credential:<target>` | Whether the selected OS-store or environment record resolves, without revealing it. Ready means local resolution, not provider authentication. | Run `papio config credentials status`; repair the binding or unlock the user credential store, then restart the daemon. Missing optional keys use keyless quotas; required-key integrations remain unavailable. |
| `source_openalex` | An enabled OpenAlex source has email and API key. | Set `email`; use `papio config credentials set sources.openalex` for account capacity. |
| `source_core` | An enabled CORE source has an API credential. | Run `papio config credentials set sources.core`, or disable the source. |
| `source_crossref-tdm` | An enabled Crossref TDM source has an API credential. | Run `papio config credentials set sources.crossref_tdm`, or disable the source. |
| `credits_<source>` | Today's metered spend is named, with the ceiling and the credentials that ceiling is shared by: `openalex has committed 12 of 5000 credits today, from 10000 reported by the provider (shared by anonymous, key-05e71a0b2f38b802)`. The fuse keeps one row per source per day, so the credential list is what the allowance is shared by, not a per-credential breakdown of the spend. Reported for every source whose `daily_credit_fraction` is non-zero, healthy or not. | A warning means today's allowance is spent and that source is parked until `00:00 UTC` — *papio* is waiting, not broken. Lower the reserve by raising `daily_credit_fraction`, or add capacity on the provider. A warning naming a changed cost schedule means the provider's prices no longer match what *papio* committed against, and egress stays closed until the new schedule is acknowledged. Before the day's first response arrives the ceiling reads as a conservative cap rather than a fraction of the provider's figure. |
