...
*Amendment 2026-08-16:* correlation with an already-established job is itself an identity decision, narrowing the original consequence that no new acceptance path exists. Picking a paper in the popup supplies correlation evidence, not authority to override conclusive front-matter identity, and every path still passes ordinary candidate validation — a conflicting conclusive DOI parks for review instead of being filed.

*Amended again 2026-08-16:* the autonomous half of that route is **withdrawn**. Binding a DOI-less capture to a uniquely qualifying pending paper without asking (`candidate_auto_bind/1`) is disabled: review found it treated a mention of a paper's identifier as the document identifying itself, so a document citing a paper — an erratum, a supplement, a journal expansion printing "Extended from DOI …" — could be filed as that paper, which is the one failure this project refuses. Such a capture parks for human identification again, exactly as the original decision specified. The popup picker and the conclusive-identity veto stand. A redesigned rule must prove a document identifies *itself*, read its evidence from one parsed front-matter structure rather than from anywhere in the page, hold an exact authority lease naming the job and the request it answers, and be measured against a real library rather than curated examples.
