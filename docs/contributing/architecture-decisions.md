...
**Context:** The inbox, the toolbar badge, and the acquisition-history page all need to notice daemon-side change. A live daemon-push design (derived topic-version fingerprints delivered over the browser bridge) was built and measured; see ADR-0005 in `dev/adr/`, kept as a record of the rejected alternative.
